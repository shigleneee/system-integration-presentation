...
const FLOW_STEPS = [
  ...
  { label:'04 Impl.',    bg:'#1a0a1f', color:C.p4, border:'#3a1a3a' },
  { label:'05 Testing',  bg:'#1f0a0a', color:C.p5, border:'#3a1a1a' },
  { label:'06 Maint.',   bg:'#0a1a1f', color:C.p6, border:'#1a3a3a' },
];

const CONNECTORS = ['Planning→Analysis','Analysis→Design','Design→Impl.','Impl.→Testing','Testing→Maint.'];
...
function EZaalDiagram() {
  ...
  return (
    <div style={{ background:C.bg, color:'#c8d8f0', fontFamily:"'IBM Plex Sans',sans-serif",
      padding:'24px 20px 36px', minHeight:'100%' }}>

      {/* Header */}
      <div style={{ textAlign:'center', marginBottom:28 }}>
        <h1 style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'1.35rem',
          letterSpacing:'0.24em', color:C.p1, textTransform:'uppercase', margin:0 }}>E-Zaal</h1>
        <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'0.62rem',
          color:C.muted, marginTop:5, letterSpacing:'0.11em' }}>
      ...
      {/* Flow bar */}
      <div style={{ display:'flex', alignItems:'center', overflowX:'auto', gap:0, marginBottom:20, paddingBottom:2 }}>
        {FLOW_STEPS.map((s,i)=>(
          <React.Fragment key={i}>
            <div style={{ flexShrink:0, textAlign:'center', padding:'5px 8px',
              fontFamily:"'IBM Plex Mono',monospace", fontSize:'0.48rem',
              letterSpacing:'0.06em', textTransform:'uppercase', borderRadius:4,
              background:s.bg, color:s.color, border:`1px solid ${s.border}` }}>{s.label}</div>
            {i<FLOW_STEPS.length-1 && <span style={{ color:C.dim, fontSize:'0.75rem', padding:'0 2px', flexShrink:0 }}>▸</span>}
          </React.Fragment>
        ))}
      </div>
        ...
        {PHASES.map((ph,i)=>(
          <div key={i}>
            <div style={{ background:C.surface, border:`1px solid ${C.border}`,
              borderLeft:`3px solid ${ph.color}`, borderRadius:9, overflow:'hidden' }}>

              {/* Header */}
              <div onClick={()=>toggle(i)} style={{ display:'grid',
                gridTemplateColumns:'46px 1fr auto', alignItems:'center', cursor:'pointer' }}>
                <div style={{ background:ph.numBg, color:ph.color, minHeight:50,
                  display:'flex', alignItems:'center', justifyContent:'center',
                  fontFamily:"'IBM Plex Mono',monospace", fontSize:'0.88rem', fontWeight:600 }}>
                  {ph.num}
                </div>
                <div style={{ padding:'11px 13px', display:'flex', alignItems:'center', gap:9 }}>
                  <span style={{ fontSize:'1.15rem', flexShrink:0 }}>{ph.icon}</span>
                  <div>
                    <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'0.68rem',
                      fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:ph.color }}>
                      {ph.name}
                    </div>
                    <div style={{ fontSize:'0.56rem', color:C.muted, marginTop:2 }}>{ph.tagline}</div>
                  </div>
                </div>
                <div style={{ padding:'0 13px', flexShrink:0, display:'flex', alignItems:'center', gap:7 }}>
                  <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'0.49rem',
                    padding:'2px 8px', borderRadius:20, whiteSpace:'nowrap',
                    background:ph.numBg, color:ph.color, border:`1px solid ${ph.tagBorder}` }}>
                    {ph.tag}
                  </span>
                  <span style={{ color:C.muted, fontSize:'0.58rem' }}>{open[i]?'▲':'▼'}</span>
                </div>
              </div>

              {open[i] && (
                <div style={{ padding:'0 13px 14px 58px' }}>{ph.body}</div>
              )}
            </div>

            {i<PHASES.length-1 && (
              <div style={{ display:'flex', alignItems:'center', height:20 }}>
                <div style={{ flex:1, height:1, background:C.border, maxWidth:'38%' }} />
                <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'0.46rem',
                  color:C.dim, padding:'0 8px', letterSpacing:'0.05em' }}>{CONNECTORS[i]}</span>
                <div style={{ flex:1, height:1, background:C.border, maxWidth:'38%' }} />
              </div>
            )}
          </div>
      ...
      <div style={{ textAlign:'center', marginTop:28, fontFamily:"'IBM Plex Mono',monospace",
        fontSize:'0.48rem', color:C.dim, letterSpacing:'0.06em' }}>
...
// ─────────────────────────────────────────────
// Slides — index 0 = Logic Flow A = SDLC diagram
// ─────────────────────────────────────────────
interface Slide {
...
const SLIDES: Slide[] = [
  { type:'component', label:'Logic Flow A',                          labelColor:'text-emerald-400', component:<EZaalDiagram /> },
  { type:'image',     label:'Нүүрний слайд',                         labelColor:'text-emerald-400', url:'/s1.png' },
  { type:'image',     label:'Асуудал ба Шийдэл',                     labelColor:'text-blue-400',    url:'/probsol.png' },
  { type:'image',     label:'Системийн тухайд',                      labelColor:'text-blue-400',    url:'/prop.png' },
  { type:'image',     label:'Функциональ шаардлага',                  labelColor:'text-purple-400',  url:'/funreq.png' },
  { type:'image',     label:'Дэлгэрэнгүй Use Case Шинжилгээ',        labelColor:'text-amber-400',   url:'/ucdef.png' },
  { type:'image',     label:'Use Case диаграм',                       labelColor:'text-amber-400',   url:'/usecase.png' },
  { type:'image',     label:'Товч Request For Proposal',              labelColor:'text-amber-400',   url:'/rfp.png' },
  { type:'image',     label:'Request For Proposal бичиг баримт',     labelColor:'text-amber-400',   url:'/rpfdoc.png' },
  { type:'image',     label:'Төгсгөл',                               labelColor:'text-amber-400',   url:'/end.png' },
];
...
export default function App() {
  ...
  const handleHotspotClick = (q: Question) => {
    setSelectedQuestion(q); setFocusedIndex(q.id); setIsBoardFocused(false); setShowAnswer(false);
    if (!q.found) {
  ...
  const handleNext = () => {
    const ni = focusedIndex === null ? 0 : (focusedIndex + 1) % questions.length;
    handleHotspotClick(questions[ni]);
  };
  const handlePrev = () => {
    const pi = focusedIndex === null ? questions.length-1 : (focusedIndex - 1 + questions.length) % questions.length;
    handleHotspotClick(questions[pi]);
  };
  const resetView = () => { setFocusedIndex(null); setIsBoardFocused(false); setSelectedQuestion(null); };
  const focusMainBoard = () => {
    setFocusedIndex(null); setIsBoardFocused(true); setSelectedQuestion(null);
    setMasterImageIndex(0); // always open on Logic Flow A (SDLC)
  ...
  const slide = SLIDES[masterImageIndex];
  ...
  return (
    <div className="fixed inset-0 bg-neutral-950 overflow-hidden font-sans text-white">
      ...
      <div ref={containerRef} className="relative w-full h-full flex items-center justify-center p-4 md:p-8">
        <motion.div animate={{ scale:zoomScale, x:`${zoomX}%`, y:`${zoomY}%` }}
          ...
          {imageUrl
            ...
            : <div className="w-full h-full bg-neutral-900 flex items-center justify-center"><p className="text-neutral-500">Failed to load environment</p></div>
          }
          <div className="absolute inset-0 pointer-events-none">
            {questions.map(q => (
              <motion.button key={q.id}
                ...
                className={cn("absolute w-6 h-6 -ml-3 -mt-3 rounded-full pointer-events-auto transition-colors z-10",
      ...
      <div className="absolute bottom-8 right-8 flex gap-4">
        <button onClick={focusMainBoard} className={cn("flex items-center gap-2 px-6 py-4 bg-neutral-900/80 backdrop-blur-md border rounded-2xl transition-all group",
        ...
        <button onClick={handlePrev} className="p-4 bg-neutral-900/80 backdrop-blur-md border border-white/10 rounded-2xl hover:bg-neutral-800 transition-colors group">
          <ChevronRight className="w-6 h-6 rotate-180 group-hover:-translate-x-1 transition-transform" />
        </button>
        <button onClick={resetView} className={cn("px-6 py-4 bg-neutral-900/80 backdrop-blur-md border rounded-2xl transition-all font-mono text-sm uppercase tracking-widest",
          focusedIndex===null&&!isBoardFocused?"border-blue-500 text-blue-500":"border-white/10 text-white hover:bg-neutral-800")}>
          Өрөөг харах
        </button>
        <button onClick={handleNext} className="p-4 bg-neutral-900/80 backdrop-blur-md border border-white/10 rounded-2xl hover:bg-neutral-800 transition-colors group">
        ...
        <motion.div initial={{ x:-20, opacity:0 }} animate={{ x:0, opacity:1 }}
          ...
          <div className="mt-6 flex items-center gap-4">
            <div className="flex-1 h-2 bg-neutral-800 rounded-full overflow-hidden">
              <motion.div initial={{ width:0 }} animate={{ width:`${(foundCount/Math.max(questions.length,1))*100}%` }}
        ...
        {showMasterImage && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-neutral-950/80 backdrop-blur-md">
            <motion.div initial={{ scale:0.9, opacity:0 }} animate={{ scale:1, opacity:1 }} exit={{ scale:0.9, opacity:0 }}
              ...
              style={{ maxWidth: slide.type==='component' ? 1100 : 960, maxHeight:'92vh' }}>

              {/* Slide area */}
              <div className="relative flex-1 overflow-hidden group" style={{ minHeight:0 }}>
                <AnimatePresence mode="wait">
                  <motion.div key={masterImageIndex}
                    initial={{ opacity:0, x:24 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-24 }}
                    transition={{ duration:0.22 }}
                    style={{ width:'100%', height: slide.type==='component' ? '76vh' : '65vh' }}>

                    {slide.type==='component' ? (
                      // ── SDLC Diagram fills the slide ──
                      ...
                      <div className="w-full h-full p-8">
                        <div className="relative w-full h-full rounded-2xl overflow-hidden border border-white/5 bg-neutral-950/50">
                          <img src={slide.url} alt={slide.label} className="w-full h-full object-contain" referrerPolicy="no-referrer" />
                          <div className="absolute bottom-6 left-6 px-4 py-2 bg-neutral-950/50 backdrop-blur-md rounded-lg border border-white/10">
                            <p className={cn("text-xs font-mono uppercase tracking-widest font-bold", slide.labelColor)}>{slide.label}</p>
                          </div>
                        </div>
                      </div>
                    )}
                ...
                {/* Carousel arrows */}
                <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none group-hover:pointer-events-auto">
                  <button onClick={()=>setMasterImageIndex(p=>(p===0?SLIDES.length-1:p-1))}
                    className="p-3 bg-neutral-950/60 hover:bg-neutral-950/90 backdrop-blur-md rounded-full text-white transition-all border border-white/10 opacity-0 group-hover:opacity-100">
                    <ChevronRight className="w-6 h-6 rotate-180" />
                  </button>
                </div>
                <div className="absolute inset-y-0 right-3 flex items-center pointer-events-none group-hover:pointer-events-auto">
                  <button onClick={()=>setMasterImageIndex(p=>(p===SLIDES.length-1?0:p+1))}
                    className="p-3 bg-neutral-950/60 hover:bg-neutral-950/90 backdrop-blur-md rounded-full text-white transition-all border border-white/10 opacity-0 group-hover:opacity-100">
              ...
              <button onClick={()=>setShowMasterImage(false)}
                className="absolute top-5 right-5 p-3 bg-neutral-950/60 hover:bg-neutral-950/90 backdrop-blur-md rounded-full text-white border border-white/10 group z-10 transition-all">
                <X className="w-5 h-5 group-hover:rotate-90 transition-transform" />
              </button>

              {/* Footer */}
              <div className="px-8 py-4 bg-neutral-950/50 border-t border-white/5 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4">
                  <p className="text-xs font-mono text-neutral-500 uppercase tracking-widest">Диаграм зураглал, ICSI314</p>
                  <div className="flex gap-1.5 items-center">
                    {SLIDES.map((_,i)=>(
                      <button key={i} onClick={()=>setMasterImageIndex(i)}
                        className={cn("rounded-full transition-all duration-300",
                          masterImageIndex===i?"w-5 h-2 bg-emerald-500":"w-2 h-2 bg-neutral-600 hover:bg-neutral-400")} />
                    ))}
                  </div>
                </div>
                <p className={cn("text-xs font-mono uppercase tracking-widest font-bold", slide.labelColor)}>{slide.label}</p>
